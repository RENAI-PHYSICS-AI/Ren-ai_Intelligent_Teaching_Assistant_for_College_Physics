...
SOURCE_DIR = PROJECT_ROOT / "教学素材" / "物理实验" / "电子荷质比"
PDF_DIR = SOURCE_DIR / "ref"
OUTPUT_STEM = "electron_em"
CHUNK_SIZE = 760
CHUNK_OVERLAP = 120

# 两份综合实验手册只导入与电子荷质比直接相关的页，避免其他实验内容干扰检索。
PDF_PAGE_RANGES: dict[str, range] = {
    "Columbia_2022_Lab_Manual_Experiment4_em.pdf": range(37, 45),
    "IITK_2024_25_Physics_Lab_Manual.pdf": range(36, 43),
}

TOPIC_BY_SOURCE = {
    "Alzate_Cardona_2019_Magnetic_Field_Visualizer.pdf": "亥姆霍兹线圈磁场标定",
    "CCNY_Lorentz_Force_Apparatus_Instructions.pdf": "电子束圆轨道法",
    "Columbia_2022_Lab_Manual_Experiment4_em.pdf": "电子束圆轨道法",
    "Gray_2024_em_Systematic_Uncertainty.pdf": "系统误差与不确定度",
    "IITK_2024_25_Physics_Lab_Manual.pdf": "亥姆霍兹线圈磁场标定",
    "LD_Didactic_P6.1.3.1_Specific_Charge.pdf": "电子束圆轨道法",
    "NIST_2022_CODATA_Full_Report.pdf": "标准值与不确定度",
    "NIST_2022_CODATA_Fundamental_Constants.pdf": "标准值与不确定度",
    "PASCO_SE-9629_em_Apparatus_Manual.pdf": "电子束圆轨道法",
    "Rochester_Experiment08_Electron_em.pdf": "电子束圆轨道法",
    "Sarapak_2025_3D_Virtual_Lab_em.pdf": "三维虚拟实验与教学",
    "Silviana_Prayogi_2023_Smartphone_em.pdf": "数字化图像测量",
    "Thomson_1897_Cathode_Rays.pdf": "历史与汤姆孙交叉场",
    "TJNU_电子比荷的测定_DH4520.pdf": "电子束圆轨道与螺旋运动",
    "UVA_Lab07_Electron_em.pdf": "电子束圆轨道法",
    "UZH_Electron_Charge_to_Mass_Ratio_Lab_Manual.pdf": "纵向磁聚焦与圆轨道",
    "William_Mary_2014_emratio_Lab_Manual.pdf": "数据线性化与拟合",
}
...
def source_language(path: Path, text: str = "") -> str:
    if path.name.startswith("TJNU_") or path.suffix.lower() == ".md":
        return "zh"
...
def markdown_sections(path: Path) -> list[tuple[str, str]]:
    ...
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if heading:
            ...
            topic = clean(re.sub(r"^\d+(?:\.\d+)*\s*", "", heading.group(1)))
            lines = [line]
        else:
            lines.append(line)
    if useful("\n".join(lines)):
...
def extract_pdf(pdftotext: str, pdfinfo: str | None, path: Path) -> tuple[list[dict], dict]:
    ...
    topic = TOPIC_BY_SOURCE.get(path.name, "电子荷质比实验")
    ...
    if not rows:
        catalog = f"电子荷质比参考文献：{source_title(path)}。主题：{topic}。该 PDF 未提取到可检索文字层，建议完成 OCR 后重新构建专题索引。"
...
def build() -> dict:
    ...
    if not pdftotext:
        raise RuntimeError("缺少 pdftotext，无法提取电子荷质比 PDF 文献")

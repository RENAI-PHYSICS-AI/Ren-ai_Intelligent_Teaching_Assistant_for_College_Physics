...
路径配置优先使用 `/` 分隔的相对路径。直接运行本目录的 Python 或 Windows 启动脚本时，相对路径以本目录为基准，不受命令行当前目录影响（例如 `PHYSICS_EXAM_MATERIALS_DIR=../考试素材`）；Rocky 的外层安装/管理脚本则先按发布目录解析配置。外部资源仍可配置绝对路径，系统命令名保留 `PATH` 查找。

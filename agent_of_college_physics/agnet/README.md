...
模型采用本地两阶段路由：普通问题直接由 Rocky 本机的 MiMo VL Miloco 7B 回答；上传图片时先由同一模型提取可见信息，再把识别文本、知识库结果和学生问题交给同一模型组织最终答案。第二阶段不会再次发送图片原始数据。Rocky 生产标识统一为 `mimo-vl-local-prod`；`manage.sh` 会按 128K 上下文、4 个并行槽检查并加载该模型，不设置 TTL，因而保持常驻。

...
class NewtonRingsIntegrationTests(unittest.TestCase):
    ...
    def test_rocky_installer_and_manager_include_newton_rings(self):
        rocky_root = APP_DIR.parent
        installer = self._source(rocky_root / "install.sh")
        manager = self._source(rocky_root / "manage.sh")
        env_example = self._source(rocky_root / "physics-assistant.env.example")

        self.assertIn(
            "for experiment in lissajous sound_speed electron_em "
            "photoelectric biprism newton_rings",
            installer,
        )
        self.assertIn('"$APP_ROOT/agnet/runtime/experiments/newton_rings.log"', installer)
        self.assertIn('PHYSICS_NEWTON_RINGS_PORT="${PHYSICS_NEWTON_RINGS_PORT:-9389}"', manager)
        self.assertIn("experiments/newton_rings/web.jl", manager)
        self.assertIn("physics-experiment:newton-rings", manager)
        self.assertIn("experiments/newton-rings/__physics_health__", manager)
        self.assertIn("PHYSICS_NEWTON_RINGS_PORT=9389", env_example)
        self.assertIn(

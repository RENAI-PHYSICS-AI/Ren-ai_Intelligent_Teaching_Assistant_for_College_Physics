...
class YoungModulusIntegrationTests(unittest.TestCase):
    ...
    def test_rocky_install_precompiles_and_prepares_the_young_modulus_service(self):
        ...
        self.assertIn('runtime/experiments/young_modulus.log', install_source)
        self.assertRegex(
            install_source,
            r"for experiment in\s+[^;\n]*\byoung_modulus\b[^;\n]*;\s*do",
        )
        self.assertIn(
            '"$APP_ROOT/agnet/experiments/$experiment/web.jl" '
            "--no-instantiate --self-test",
            install_source,
        )

    def test_rocky_manage_exports_stops_and_checks_port_9390(self):
        manage_source = self._source(ROCKY_ROOT / "manage.sh")

        self.assertIn(
            'PHYSICS_YOUNG_MODULUS_PORT="${PHYSICS_YOUNG_MODULUS_PORT:-9390}"',
            manage_source,
        )
        self.assertIn(
            "PHYSICS_YOUNG_MODULUS_UPSTREAM="
            '"${PHYSICS_YOUNG_MODULUS_UPSTREAM:-http://127.0.0.1:'
            '$PHYSICS_YOUNG_MODULUS_PORT}"',
            manage_source,
        )
        self.assertIn("experiments/young_modulus/web.jl", manage_source)
        self.assertIn(
            'http://127.0.0.1:$PHYSICS_YOUNG_MODULUS_PORT/'
            "__physics_health__",
            manage_source,
        )
        self.assertIn(
            "http://127.0.0.1:8501/experiments/young-modulus/"
            "__physics_health__",
            manage_source,
        )
        self.assertIn("physics-experiment:young-modulus", manage_source)

    def test_rocky_environment_example_exposes_only_the_loopback_upstream(self):
        env_source = self._source(ROCKY_ROOT / "physics-assistant.env.example")

        self.assertIn("PHYSICS_YOUNG_MODULUS_PORT=9390", env_source)
        self.assertIn(
            "PHYSICS_YOUNG_MODULUS_UPSTREAM=http://127.0.0.1:9390",
            env_source,
        )
        self.assertNotIn("PHYSICS_YOUNG_MODULUS_UPSTREAM=http://0.0.0.0", env_source)
